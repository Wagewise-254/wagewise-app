...
import require$$1$4, { app, session, BrowserWindow, dialog } from "electron";
...
function inspectNode(object2, objects2, duplicatesIndexes) {
  if (object2 !== null && typeof object2 === "object") {
    const index = objects2.indexOf(object2);
    if (index !== -1) {
      ...
      if (Array.isArray(object2)) {
        for (let i = 0, length = object2.length; i < length; i += 1) {
          inspectNode(object2[i], objects2, duplicatesIndexes);
        }
      } else {
        const objectKeyList = Object.keys(object2);
        for (let i = 0, length = objectKeyList.length; i < length; i += 1) {
          inspectNode(object2[objectKeyList[i]], objects2, duplicatesIndexes);
        }
      }
    }
  }
...
const createMainWindow = () => {
  mainWindow = new BrowserWindow({
    icon: path$n.join(process.env.VITE_PUBLIC, "/icons/favicon.ico"),
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    mainWindow.loadFile(path$n.join(RENDERER_DIST, "index.html"));
  }
};
...
const setupAutoUpdater = () => {
  ...
  main$2.autoUpdater.autoDownload = false;
  main$2.autoUpdater.allowPrerelease = isBeta;
    ...
    if (mainWindow) {
      dialog.showMessageBox(mainWindow, {
        ...
        if (result.response === 0) {
          main$2.autoUpdater.downloadUpdate();
          if (mainWindow) {
            dialog.showMessageBox(mainWindow, {
              type: "info",
  ...
  main$2.autoUpdater.on("update-not-available", () => {
  ...
  main$2.autoUpdater.on("error", (error2) => {
    console.error("❌ Update Error:", error2);
    if (mainWindow) {
      dialog.showErrorBox("Update Error", `Failed to check for updates: ${error2.message}`);
    }
  });
