    ...
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className="relative h-9 w-9 rounded-full text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-colors"
        >
          <Bell className="h-5 w-5" strokeWidth={1.75} />
          {unreadCount > 0 && (
            <Badge
              variant="destructive"
              className="absolute -top-1 -right-1 h-4 min-w-4 px-1 text-[10px] flex items-center justify-center bg-red-500 text-white rounded-full border border-white"
            >
      ...
      <DropdownMenuContent
        align="end"
        className="w-96 p-0 rounded-xl shadow-xl border border-slate-200 bg-white"
        sideOffset={8}
      >
        {/* Header */}
        ...
        <ScrollArea className="h-100">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-32 p-4 text-center">
              <Bell className="h-7 w-7 text-slate-300 mb-2" strokeWidth={1.5}/>
