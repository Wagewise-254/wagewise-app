// src/components/company/benefits/ImportAllowancePreviewDialog.tsx
...
const getRecipientDisplay = (data: AllowanceImportData) => {
...
export default function ImportAllowancePreviewDialog({
  companyId,
  isOpen,
  onClose,
  onSuccess,
    ...
    async (fileToAnalyze: File) => {
      ...
      try {
        const response = await axios.post(
          `${API_BASE_URL}/company/${companyId}/allowances/import/preview`,
          formData,
          {
            headers: {
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      // Prepare allowances - include valid records
      const allowancesToImport = preview.valid.map((v) => ({
        ...v.data,
        row: v.row,
      }));

      // If skipDuplicates is false and we have selected overrides, include them
      let allAllowances = [...allowancesToImport];
      ...
      if (!skipDuplicates && overrideRowIds.length > 0) {
        ...
        allAllowances = [...allAllowances, ...duplicatesToImport];
      }
      ...
      if (response.data.results) {
        ...
        if (updated && updated.length > 0) {
          toast.success(`${updated.length} allowance(s) updated successfully`);
        }
      ...
      toast.error("Failed to import allowances");
    } finally {
      setImporting(false);
    }
  };
      ...
      <>
        ...
        {!skipDuplicates && selectedOverrides.size > 0 && (
          <div className="mt-3 p-2 bg-blue-50 rounded text-xs text-blue-700">
            <strong>{selectedOverrides.size}</strong> allowance(s) will be
        ...
        {skipDuplicates && (
          <div className="mt-3 p-2 bg-slate-50 rounded text-xs text-slate-500">
            ...
            rows to override existing allowances.
          </div>
        )}
      </>
    );
    ...
    <Dialog open={isOpen} onOpenChange={resetAndClose}>
      <DialogContent className="sm:max-w-5xl max-h-[90vh] flex flex-col p-0">
        <DialogHeader className="p-6 pb-4 border-b shrink-0">
          ...
          <DialogDescription>
            Upload an Excel file with allowance assignments. The system will
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-500 mb-3">
                        Get the Excel template with dropdowns for allowance
