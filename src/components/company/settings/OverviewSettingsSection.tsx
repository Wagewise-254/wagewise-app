...
export default function OverviewSettingsSection() {
  ...
  const {
    ...
    loading,
    error,
  } = useCompanyStore(); // removed this
  const [selectedCompanyId, setSelectedCompanyId] = useState<string | null>(
    null,
  );
  const [logoFile, setLogoFile] = useState<File | null>(null);
  const [companyStatus, setCompanyStatus] = useState<string>("active");
  const [transferRecipientEmail, setTransferRecipientEmail] =
    useState<string>("");
  ...
  const handleUpdate = async (field: "logo" | "status") => {
    ...
    try {
      ...
      await updateCompany(selectedCompanyId, formData);
      toast.success(
        `${field === "logo" ? "Logo" : "Status"} updated successfully!`,
      );
    } catch (err) {
        ...
        <>
          ...
          <div className="bg-white rounded-xl shadow p-6 space-y-8 mb-8">
            ...
            <Button
              ...
              className="cursor-pointer bg-[#7F5EFD]"
              disabled={underMaintenance || loading}
            >
              Save Basic Info
            </Button>
          </div>
          ...
          <div className="bg-white rounded-xl shadow p-6 space-y-4 mb-8">
            ...
            <Button
              ...
              disabled={underMaintenance || !logoFile || loading}
            >
              Save Logo
            </Button>
          </div>
          ...
          <div className="bg-white rounded-xl shadow p-6 space-y-4 mb-8">
            ...
            {currentCompany.status !== "active" && (
              <p className="text-xs text-muted-foreground italic">
                Your company is currently under review. Please contact support
          ...
          <div className="bg-white rounded-xl shadow p-6 space-y-4">
            ...
            <p className="text-sm text-gray-600">
              Transfer ownership of this company to another user. This action
              cannot be undone and will remove the company from your account.
            </p>
            ...
            <Dialog>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>
                    Transfer Ownership of "{currentCompany.business_name}"
                  </DialogTitle>
                  <DialogDescription>
                    This is a permanent action. Are you sure you want to
                    transfer this company?
                  </DialogDescription>
                </DialogHeader>
                <div className="py-4">
                  <p className="mb-2">
                    Enter the email of the new owner. They must have an existing
                    account.
                  </p>
                  <Input
                ...
                <DialogFooter>
                  ...
                  <Button
                    ...
                    disabled={loading || !transferRecipientEmail}
                  >
