...
export default function EditDeductionDialog({ deduction, companyId, isOpen, onClose, onUpdated }: Props) {
  const { session } = useAuthStore();
  const [name, setName] = useState(deduction.name);
  const [description, setDescription] = useState(deduction.description);
  const [isTaxDeductible, setIsTaxDeductible] = useState(deduction.is_tax_deductible);
